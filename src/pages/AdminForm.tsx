...
export default function AdminForm() {
  ...
  const fetchAdmin = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to load admin",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">
            {isCreate ? "Add New Admin" : isEditRoute ? "Edit Admin" : "View Admin"}
          </h1>
          <p className="text-muted-foreground">
            {isCreate ? "Enter admin details" :
              isEditRoute ? "Update admin details" : "Read-only admin details"}
          </p>
        </div>
      </div>
              ...
              {!isViewOnly && (
                <>
                  ...
                  <Button type="submit" disabled={loading}>
                    {loading ? "Saving..." : isEditRoute ? "Update Admin" : "Create Admin"}
                  </Button>
                </>
              )}
            </div>
